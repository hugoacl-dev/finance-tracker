...
# ═══════════════════════════════════════
# INDEPENDÊNCIA FINANCEIRA (Wave 4.6)
# ═══════════════════════════════════════
def calcular_independencia_financeira(
    custo_mensal: float,
    aporte_mensal: float,
    ...
    """
    Calcula tempo para independência financeira.
    Regra dos 4%: patrimônio necessário = custo_mensal × 12 / 0.04
    """
    patrimonio_necessario = (custo_mensal * 12) / 0.04
    faltante = max(0, patrimonio_necessario - patrimonio_atual)
    ...
    if aporte_mensal <= 0:
        return {
            ...
            "meses": float("inf"),
            "anos": float("inf"),
            "label": "Impossível sem aporte",
        }

    # Cálculo com juros compostos mensais
    r_mensal = (1 + rendimento_anual) ** (1 / 12) - 1
    if r_mensal > 0:
        import math
        meses = math.log(
            (faltante * r_mensal + aporte_mensal) / aporte_mensal
        ) / math.log(1 + r_mensal)
    else:
        meses = faltante / aporte_mensal
    ...
    return {
        ...
        "meses": round(meses),
        "anos": round(anos, 1),
        "label": f"{int(anos)} anos e {int(meses % 12)} meses",
    }
